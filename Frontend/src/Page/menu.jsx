...
const ParticleCanvas = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      pts.forEach((p) => {
        p.y -= p.speed;
        p.x += p.drift + Math.sin(t + p.pulse) * 0.25;
        p.pulse += 0.008;
        if (p.y < -4) {
          p.y = canvas.height + 4;
      ...
      embers.forEach((e) => {
        e.y -= e.speed;
        e.x += e.drift + Math.sin(t * 0.7 + e.pulse) * 0.4;
        e.pulse += 0.006;
        if (e.y < -6) {
          e.y = canvas.height + 6;
        ...
        ctx.fillStyle = `rgba(255,230,120,${Math.min(a * 1.5, 1)})`;
        ctx.fill();
      });
  ...
  return (
    <canvas ref={canvasRef} className="menu-bg__canvas" aria-hidden="true" />
  );
};

// ─── Helpers (🌟 ĐÃ ĐỒNG BỘ SANG CHUỖI CHỮ TIẾNG VIỆT) ────────────────────────
        ...
        {item.image_url ? (
          ...
          >
            ...
            >
              ...
              />
              <path
                d="M76 4 L76 20 M76 4 L60 4"
                stroke="rgba(212,175,55,0.4)"
                strokeWidth="1.5"
                strokeLinecap="round"
              />
              <path
                d="M4 76 L4 60 M4 76 L20 76"
                stroke="rgba(212,175,55,0.4)"
                strokeWidth="1.5"
                strokeLinecap="round"
              />
              <path
                d="M76 76 L76 60 M76 76 L60 76"
                stroke="rgba(212,175,55,0.4)"
                strokeWidth="1.5"
                strokeLinecap="round"
              />
            ...
            <span className="menu-card__emoji">
        ...
        <div className="modal__image-wrap">
          {item.image_url ? (
            ...
            >
              ...
              >
                <path
                  d="M8 8 L8 30 M8 8 L30 8"
                  stroke="rgba(212,175,55,0.45)"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                />
                <path
                  d="M112 8 L112 30 M112 8 L90 8"
                  stroke="rgba(212,175,55,0.45)"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                />
                <path
                  d="M8 112 L8 90 M8 112 L30 112"
                  stroke="rgba(212,175,55,0.45)"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                />
                <path
                  d="M112 112 L112 90 M112 112 L90 112"
                  stroke="rgba(212,175,55,0.45)"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                />
...
export default function Menu() {
  ...
  const [activeCategory, setActiveCategory] = useState(""); // 🌟 ĐỔI THÀNH CHUỖI RỖNG
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.error(err);
        setError("Không tải được dữ liệu món ăn");
      }
      try {
        const res = await fetch("http://localhost:5000/api/admin/categories");
        if (!res.ok) throw new Error(`Categories API lỗi: ${res.status}`);
        const d = await res.json();
        const actualCategories = d.data || d || [];

        if (Array.isArray(actualCategories)) {
          setCategories([
            { id: "", name: "Tất cả", icon: "👑" }, // id rỗng đại diện cho Tất cả
            ...actualCategories,
          ]);
        }
      } catch (err) {
  ...
  const filtered = menuItems.filter((item) => {
    // 🌟 ĐÃ SỬA LOGIC LỌC THEO TÊN DANH MỤC CHUỖI CHỮ TIẾNG VIỆT
    if (activeCategory !== "" && item.category_name !== activeCategory)
      return false;
      ...
      <header className="menu-hero">
        <div className="menu-hero__ornament" aria-hidden="true">
          ...
          >
            ...
            />
            <polygon
              points="134,8 142,16 134,24 126,16"
            ...
            />
            <polygon
              points="186,8 194,16 186,24 178,16"
              stroke="rgba(212,175,55,0.7)"
              strokeWidth="1"
              fill="rgba(212,175,55,0.12)"
            />
        ...
        <p className="menu-hero__subtitle">
          Trải nghiệm ẩm thực đỉnh cao — mỗi món là một tác phẩm
        </p>
      </header>
      ...
      <nav className="menu-tabs" aria-label="Danh mục">
        <div className="menu-tabs__track">
          {categories.map((cat) => (
            <button
              key={cat.id || cat.name}
              /* 🌟 ĐÃ SỬA: Logic so sánh class Active dựa trên chuỗi chữ */
              className={`menu-tab ${
                (cat.name === "Tất cả" && activeCategory === "") ||
                activeCategory === cat.name
                  ? "menu-tab--active"
                  : ""
              }`}
              /* 🌟 ĐÃ SỬA: Bấm vào tab gán thẳng name thay vì id số */
              onClick={() =>
                setActiveCategory(cat.name === "Tất cả" ? "" : cat.name)
              }
            >
