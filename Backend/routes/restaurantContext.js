...
export async function getRestaurantContext() {
  ...
  try {
    const kb     = await new sql.Request().query(`SELECT category, question_key, answer_content FROM knowledge_base ORDER BY category, question_key`);
    const menu   = await new sql.Request().query(`SELECT c.name AS category_name, m.item_name, m.description, m.price, m.ingredients, m.allergy_warnings, m.is_best_seller, m.is_spicy, m.is_vegetarian, m.serving_size FROM menu m LEFT JOIN categories c ON m.category_id = c.id ORDER BY c.name, m.item_name`);
    const styles = await new sql.Request().query(`SELECT style_name, description, bestseller FROM table_styles ORDER BY bestseller DESC, style_name`);
    const tables = await new sql.Request().query(`SELECT rt.table_number, rt.location, rt.capacity, ts.style_name FROM restaurant_tables rt LEFT JOIN table_styles ts ON rt.style_id = ts.id WHERE rt.status != 'maintenance' ORDER BY rt.table_number`);
...
Chỉ trả lời dựa trên thông tin bên dưới. Nếu khách hỏi ngoài phạm vi này, đề nghị liên hệ trực tiếp nhà hàng.
...
═══════════════════════════════\n`;
    ...
    for (const [cat, items] of Object.entries(kbByCategory)) {
      prompt += `\n## ${cat}\n`;
      for (const item of items) prompt += `- ${item.question_key}: ${item.answer_content}\n`;
    }
...
═══════════════════════════════\n`;
    ...
    return "Bạn là trợ lý tư vấn của nhà hàng. Hãy hỗ trợ khách hàng lịch sự. Hiện tại không thể tải dữ liệu, hãy đề nghị khách liên hệ trực tiếp.";
  }
}
